...
REGULAREXPRESSION(TW_AtRegularExpression, @"\\[#&@.*?\\]")

//@"#([^\\#|.]+)#"
//REGULAREXPRESSION_OPTION(PoundSignRegularExpression, @"#([\\u4e00-\\u9fa5\\w\\-]+)#", NSRegularExpressionCaseInsensitive)  // unused
...
@property (nonatomic, assign) NSUInteger lengthOffset;
@property (nonatomic, strong) NSMutableArray *atResults;
@property (nonatomic, strong) NSMutableArray *urlResults;
...
- (void)commonInit {
    ...
    UIColor *commonLinkColor = [UIColor colorWithRed:69.f/ 255.f green:121.f / 255.f blue:251.f / 255.f alpha:1.000];
...
#pragma mark - main
/**返回经过at识别处理的at字符串 */
- (NSString *)atStringTextWithEmojiText:(NSString *)emojiText
{
    NSMutableString *atStrintText = [[NSMutableString alloc] initWithString:emojiText];
    NSMutableAttributedString *mutableAttributedString = [[NSMutableAttributedString alloc]initWithString:emojiText];
    NSRange stringRange = NSMakeRange(0, mutableAttributedString.length);
    [kTW_AtRegularExpression() enumerateMatchesInString:mutableAttributedString.string
                                                options:NSMatchingWithTransparentBounds
                                                  range:stringRange usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop)
     {
         NSRange range = result.range;
         
         range.location -= _lengthOffset;
         
//         NSString *atString = [atStrintText substringWithRange:range];
         NSString *codeString = [atStrintText substringWithRange:NSMakeRange(range.location + 4, range.length - 5)];
         NSString *userId = @"";
         
         // 如果是新的处理方式
         NSArray *spareStrArray = [codeString componentsSeparatedByString:@"|"];
         if (spareStrArray.count == 3) {         // 新的样式
             userId = spareStrArray[0];
         } else if (spareStrArray.count == 1) {  // 老的样式
             userId = codeString;
         }
         
//         NSString *atFriendName = [[TWGroupAtFriendHandle sharedInstance] getAtFriendName:userId];
//
//         if (![atFriendName isEqualToString:@""]) {
//             atFriendName = [NSString stringWithFormat:@"@%@", atFriendName];
//         }
//         
//         [atStrintText replaceCharactersInRange:range withString:atFriendName];
//         range.length = atFriendName.length;
         
         _lengthOffset += result.range.length - range.length;
         
         /*
         atString = [atStrintText substringWithRange:range];
         ...
         [self.atResults addObject:aResult];
          */
     }];
    _lengthOffset = 0;
    return atStrintText;
...
- (NSString *)urlStringTextWithEmojiText:(NSString *)emojiText
{
    NSMutableString *atStrintText = [[NSMutableString alloc] initWithString:emojiText];
    NSMutableAttributedString *mutableAttributedString = [[NSMutableAttributedString alloc]initWithString:emojiText];
    NSRange stringRange = NSMakeRange(0, mutableAttributedString.length);
    [kURLRegularExpression() enumerateMatchesInString:mutableAttributedString.string
                                              options:NSMatchingWithTransparentBounds
                                                range:stringRange usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop)
     {
         NSRange range = result.range;
         ...
         //添加链接
         NSString *atString = [emojiText substringWithRange:result.range];
         NSString *actionString = [NSString stringWithFormat:@"url->%@", atString];
         ...
         [self.urlResults addObject:aResult];
...
- (NSMutableArray *)atResults
{
    if (!_atResults) {
        _atResults = [NSMutableArray new];
    }
    return _atResults;
}

- (NSMutableArray *)urlResults
{
    if (!_urlResults) {
        _urlResults = [NSMutableArray new];
    }
...
//    NSMutableArray *tempAtResults = [[NSMutableArray alloc] initWithArray:self.atResults];
    
    for (NSTextCheckingResult *result in emojis) {
        NSRange range = result.range;
//        NSMutableArray *tempArray = [NSMutableArray new];
        
//        for (int i = 0; i < tempAtResults.count; i++) {
//            NSTextCheckingResult *pAtResult = [tempAtResults objectAtIndex:i];
//            NSTextCheckingResult *atResult = [self.atResults objectAtIndex:i];
//
//            if (pAtResult.range.location > range.location) {
//                NSRange atRange = atResult.range;
//                atRange.location -= (range.length - 1);
//                NSTextCheckingResult *aResult = [NSTextCheckingResult correctionCheckingResultWithRange:atRange replacementString:atResult.replacementString];
//                [tempArray addObject:aResult];
//            }else {
//                [tempArray addObject:atResult];
//            }
//        }
//        
//        if (tempArray.count != 0) {
//            self.atResults = tempArray;
//        }
...
//        [self.atResults removeAllObjects];
//    }
    NSMutableAttributedString *mutableAttributedString = nil;
    NSString *pEmojiText = nil;
//    if ([[TWGroupAtFriendHandle sharedInstance] isGroupSession]) {
//        pEmojiText = [self atStringTextWithEmojiText:emojiText];
//    }else {
        pEmojiText = emojiText;
//    }
    
    pEmojiText = [self urlStringTextWithEmojiText:pEmojiText];

    if (self.disableEmoji) {
        mutableAttributedString = [[NSMutableAttributedString alloc]initWithString:pEmojiText];
    }else{
        mutableAttributedString = [self mutableAttributeStringWithEmojiText:pEmojiText];
    }
...
didSelectLinkWithTextCheckingResult:(NSTextCheckingResult *)result;
{
    if (result.resultType == NSTextCheckingTypeCorrection) {
        ...
        // 先只支持url
        if ([result.replacementString hasPrefix:kURLActions[0]]) {
            NSString *content = [result.replacementString substringFromIndex:kURLActions[0].length];
            if(self.emojiDelegate&&[self.emojiDelegate respondsToSelector:@selector(mlEmojiLabel:didSelectLink:withType:)]){
                //type的数组和i刚好对应
                [self.emojiDelegate mlEmojiLabel:self didSelectLink:content withType:0];
            }
        }
