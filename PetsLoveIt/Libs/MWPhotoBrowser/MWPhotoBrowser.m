...
- (void)performLayout {
    ...
    _doneButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"完成", nil) style:UIBarButtonItemStylePlain target:self action:@selector(finishSelectButtonPressed:)];
...
- (void)finishSelectButtonPressed:(id)sender
{
    if (_delegate && [_delegate respondsToSelector:@selector(photoBrowserDidFinishSelectImageWithBrowser:)]) {
        [_delegate photoBrowserDidFinishSelectImageWithBrowser:self];
    }
}
...
- (BOOL)setPhotoSelected:(BOOL)selected atIndex:(NSUInteger)index {
    if (_displaySelectionButtons) {
        if ([self.delegate respondsToSelector:@selector(photoBrowser:photoAtIndex:selectedChanged:)]) {
            BOOL isSucess = [self.delegate photoBrowser:self photoAtIndex:index selectedChanged:selected];
            if (isSucess) {  // 如果返回YES，则添加成功设置
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            if (self.displaySelectionButtons) {
                ...
                [selectedButton setImage:[UIImage imageNamed:@"photo_unselect_img.png"] forState:UIControlStateNormal];
                [selectedButton setImage:[UIImage imageNamed:@"photo_select_img.png"] forState:UIControlStateSelected];
...
- (CGRect)frameForSelectedButton:(UIButton *)selectedButton atIndex:(NSUInteger)index {
    ...
    CGFloat originY = [self areControlsHidden] ? pageFrame.size.height + 10 : pageFrame.size.height - 40;
    
    CGRect captionFrame = CGRectMake(pageFrame.origin.x + pageFrame.size.width - 50,
                                     originY,
                                     35,
                                     35);
    return CGRectIntegral(captionFrame);
...
- (void)updateRightBarItemTitle {
    NSString *tempBarTitle = NSLocalizedString(@"完成", @"完成");
    
    NSString *barTitle = tempBarTitle;
    NSInteger selectedCount = 0;
    if (_delegate && [_delegate respondsToSelector:@selector(numberOfPhotosSelectedInPhotoBrowser:)]) {
        selectedCount = [_delegate numberOfPhotosSelectedInPhotoBrowser:self];
    }
    if (selectedCount > 0) {
        barTitle = [NSString stringWithFormat:@"%@(%ld)", tempBarTitle, (long)selectedCount];
    }
    
    [_doneButton setTitle:barTitle];
}
    ...
    [UIView animateWithDuration:animationDuration animations:^(void) {
        ...
        for (MWZoomingScrollView *page in _visiblePages) {
            if (page.selectedButton) {
                ...
                CGRect newFrame = page.selectedButton.frame;
                newFrame.origin.y = hidden ? page.frame.size.height + 10 : page.frame.size.height - 40;
                v.frame = newFrame;
            }
        }
