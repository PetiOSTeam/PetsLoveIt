    ...
    for (int row = 0; row < maxRow; row++) {
        for (int col = 0; col < maxCol; col++) {
            ...
            if (index < [_faces count]) {
                ...
                //[button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"e_%d",(row * maxCol + col+1)]] forState:UIControlStateNormal];
...
-(void)selected:(UIButton*)bt
{
    if (bt.tag == 10000 && _delegate) {
        ...
        NSString *str = [NSString stringWithFormat:@"[#&%ld]",(long)bt.tag+1];
        if (_delegate) {
