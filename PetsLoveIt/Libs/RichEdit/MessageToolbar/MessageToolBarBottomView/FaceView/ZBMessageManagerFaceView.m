...
- (void)setup{
    ...
    for (int i= 0;i<Pages;i++) {
        ZBFaceView *faceView = [[ZBFaceView alloc]initWithFrame:CGRectMake(i*CGRectGetWidth(self.bounds),5.0f,CGRectGetWidth(self.bounds),CGRectGetHeight(scrollView.bounds)) forIndexPath:i];
    ...
    sectionBar.hiddenSendButton = self.hiddenSendButton;
...
#pragma mark - **** ZBExpressionSectionBarDelegate ****

- (void)sendEmojiWithBar:(ZBExpressionSectionBar *)sectionBar
{
    if ([_delegate respondsToSelector:@selector(sendEmoji)]) {
        [_delegate sendEmoji];
    }
}
...
{
    int page = scrollView.contentOffset.x/mScreenWidth;
