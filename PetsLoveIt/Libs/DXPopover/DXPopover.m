...
+ (DXPopover*)sharedView {
    static dispatch_once_t once;
    static DXPopover *sharedView;
    dispatch_once(&once, ^ { sharedView = [[DXPopover alloc] init]; });
    return sharedView;
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.sideEdge = 10.0;
        self.maskType = DXPopoverMaskTypeNone;
...
- (void)showAtPoint:(CGPoint)point popoverPostion:(DXPopoverPosition)position withContentView:(UIView *)contentView inView:(UIView *)containerView
{
    ...
    switch (self.maskType) {
        case DXPopoverMaskTypeBlack:
            maskColor = [UIColor colorWithWhite:0.0 alpha:0.8];
            break;
...
+ (void)showAtView:(UIView *)atView atViewOffsetX:(CGFloat)offsetX popoverPostion:(DXPopoverPosition)position withContentView:(UIView *)contentView inView:(UIView *)containerView{
    [[self sharedView] dismiss];
    [[self sharedView] showAtView:atView atViewOffsetX:offsetX  popoverPostion:position withContentView:(UIView *)contentView inView:containerView];
}

+ (void)showAtView:(UIView *)atView atViewOffsetX:(CGFloat)offsetX atViewOffsetY:(CGFloat)offsetY popoverPostion:(DXPopoverPosition)position withContentView:(UIView *)contentView inView:(UIView *)containerView{
    [[self sharedView] dismiss];
    [[self sharedView] showAtView:atView atViewOffsetX:offsetX  atViewOffsetY:(CGFloat)offsetY   popoverPostion:position withContentView:(UIView *)contentView inView:containerView];
}
...
//    }
    ...
    dxP = DXPopoverPositionUp;
     atPoint.x = offsetX;
    atPoint.y = offsetY- betweenArrowAndAtView;
...
- (void)show
{
    ...
    self.contentView.backgroundColor = mRGBColor(51, 51, 51);
...
- (void)drawRect:(CGRect)rect
{
    ...
    UIColor *contentColor = self.contentView.backgroundColor ? : [UIColor blackColor];
