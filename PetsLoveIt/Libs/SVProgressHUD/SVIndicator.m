...
+ (SVIndicator*)sharedView {
    static dispatch_once_t once;
    static SVIndicator *sharedView;
    
    dispatch_once(&once, ^ { sharedView = [[self alloc] initWithFrame:[[UIScreen mainScreen] bounds]]; });
    return sharedView;
}
//返回系统默认样式的UIActivityIndicator
+ (SVIndicator*)sharedIndicatorView {
    static dispatch_once_t once;
    static SVIndicator *sharedView;
    
    dispatch_once(&once, ^ {
        sharedView = [[self alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
        sharedView.spinnerView.activityIndicatorViewStyle=UIActivityIndicatorViewStyleGray;
        sharedView.spinnerView.color=[UIColor grayColor];
        [sharedView.hudView setBackgroundColor:[UIColor clearColor]];
        [sharedView.stringLabel setTextColor:[UIColor lightGrayColor]];
        sharedView.stringLabel.shadowColor=[UIColor clearColor];
    });
    return sharedView;
}
...
/*显示一个“正在载入”的indicator*/
+ (void)showIndicator{
    [[self sharedIndicatorView] showProgress:-1 status:@"正在载入..." maskType:SVProgressHUDMaskTypeNone];
}
+ (void)showIndicatorMaskType:(SVProgressHUDMaskType)maskType {
    [[self sharedIndicatorView] showProgress:-1 status:@"正在载入..." maskType:maskType];
}
+ (void)dismissIndicator{
    
    if ([self isVisible]) {
        [[self sharedIndicatorView] dismiss];
    }
}

+ (void)showWithMaskType:(SVProgressHUDMaskType)maskType {
    
    [[self sharedView] showProgress:-1 status:@"正在载入..." maskType:maskType];
...
+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = [[SVIndicator sharedView] displayDurationForString:string];
...
- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    self.fadeOutTimer = [NSTimer timerWithTimeInterval:kSVDismissDuration target:self selector:@selector(dismiss) userInfo:nil repeats:NO];
...
+ (BOOL)isVisible {
    return ([self sharedView].alpha == 1||[self sharedIndicatorView].alpha==1);
}
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
        ...
        [stringLabel setTextColor:[UIColor lightGrayColor]];
