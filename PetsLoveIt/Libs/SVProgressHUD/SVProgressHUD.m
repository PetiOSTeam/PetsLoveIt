...
+ (void)showSuccessWithStatus:(NSString *)string displayInterval:(NSTimeInterval)displayInterval{
    [self showImage:[[self sharedView] hudSuccessImage] status:string displayInterval:displayInterval];
...
+ (void)showImage:(UIImage *)image status:(NSString *)string displayInterval:(NSTimeInterval)displayInterval{
...
- (void)timeOutDismiss {
    [self dismiss];
    [self showImage:[self hudErrorImage] status:@"亲，请求超时，请稍后重试" duration:2];
}
