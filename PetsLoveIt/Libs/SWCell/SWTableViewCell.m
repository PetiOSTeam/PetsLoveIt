...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if([keyPath isEqualToString:kTableViewPanState] && object == _tableViewPanGestureRecognizer)
    {
        if(_tableViewPanGestureRecognizer.state == UIGestureRecognizerStateBegan)
        {
            ...
            {
                if ([self.swdelegate respondsToSelector:@selector(swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:)])
                {
                    if([self.swdelegate swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:self])
                    {
...
- (void)rightUtilityButtonHandler:(id)sender
{
    ...
    if ([self.swdelegate respondsToSelector:@selector(swipeableTableViewCell:didTriggerRightUtilityButtonWithIndex:)])
    {
        [self.swdelegate swipeableTableViewCell:self didTriggerRightUtilityButtonWithIndex:utilityButtonIndex];
    }
}
...
- (void)leftUtilityButtonHandler:(id)sender
{
    SWUtilityButtonTapGestureRecognizer *utilityButtonTapGestureRecognizer = (SWUtilityButtonTapGestureRecognizer *)sender;
    NSUInteger utilityButtonIndex = utilityButtonTapGestureRecognizer.buttonIndex;
    if ([self.swdelegate respondsToSelector:@selector(swipeableTableViewCell:didTriggerLeftUtilityButtonWithIndex:)])
    {
        [self.swdelegate swipeableTableViewCell:self didTriggerLeftUtilityButtonWithIndex:utilityButtonIndex];
    }
}
...
- (void)hideUtilityButtonsAnimated:(BOOL)animated
{
    if (_cellState != kCellStateCenter)
    {
        ...
        if ([self.swdelegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:)])
        {
            [self.swdelegate swipeableTableViewCell:self scrollingToState:kCellStateCenter];
        }
    }
}
...
- (void)showLeftUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != kCellStateLeft)
    {
        ...
        if ([self.swdelegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:)])
        {
            [self.swdelegate swipeableTableViewCell:self scrollingToState:kCellStateLeft];
        }
    }
}
...
- (void)showRightUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != kCellStateRight)
    {
        ...
        if ([self.swdelegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:)])
        {
            [self.swdelegate swipeableTableViewCell:self scrollingToState:kCellStateRight];
        }
    }
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if ([self.swdelegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:)])
    {
        [self.swdelegate swipeableTableViewCell:self scrollingToState:_cellState];
    }
    ...
    if (_cellState != kCellStateCenter)
    {
        if ([self.swdelegate respondsToSelector:@selector(swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:)])
        {
            for (SWTableViewCell *cell in [self.containingTableView visibleCells]) {
                if (cell != self && [cell isKindOfClass:[SWTableViewCell class]] && [self.swdelegate swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:cell]) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView.contentOffset.x > [self leftUtilityButtonsWidth])
    {
        ...
        {
            if (self.swdelegate && [self.swdelegate respondsToSelector:@selector(swipeableTableViewCell:canSwipeToState:)])
            {
                BOOL shouldScroll = [self.swdelegate swipeableTableViewCell:self canSwipeToState:kCellStateRight];
                if (!shouldScroll)
                {
    ...
    else
    {
        ...
        if ([self leftUtilityButtonsWidth] > 0)
        {
            if (self.swdelegate && [self.swdelegate respondsToSelector:@selector(swipeableTableViewCell:canSwipeToState:)])
            {
                BOOL shouldScroll = [self.swdelegate swipeableTableViewCell:self canSwipeToState:kCellStateLeft];
                if (!shouldScroll)
                {
                    scrollView.contentOffset = CGPointMake([self leftUtilityButtonsWidth], 0);
                }
    ...
    if (self.swdelegate && [self.swdelegate respondsToSelector:@selector(swipeableTableViewCell:didScroll:)]) {
        [self.swdelegate swipeableTableViewCell:self didScroll:scrollView];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.swdelegate && [self.swdelegate respondsToSelector:@selector(swipeableTableViewCellDidEndScrolling:)]) {
        [self.swdelegate swipeableTableViewCellDidEndScrolling:self];
    }
}
