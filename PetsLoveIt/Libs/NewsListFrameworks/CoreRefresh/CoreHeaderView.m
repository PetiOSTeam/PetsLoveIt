...
-(void)stateSuccessedResultNoMoreData{
    if (self.customeEndStatusMsg) {
        [self updateInterFaceForStatusWithMessage:self.customeEndStatusMsg];
    }else
...
- (void)endRefreshing:(BOOL)now{
    ...
    self.showView.refreshing=NO;
    self.showView.progress=1.0f;
    CGFloat timeInterVal=now?.01:1.0f;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(timeInterVal * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [UIView animateWithDuration:.25f animations:^{
            ...
            self.state=CoreHeaderViewRefreshStateNorMal;
            //回调处理
            if ([self.delegate respondsToSelector:@selector(headerViewDidEndRefresh)]) {
                [self.delegate headerViewDidEndRefresh];
            }
        }];
