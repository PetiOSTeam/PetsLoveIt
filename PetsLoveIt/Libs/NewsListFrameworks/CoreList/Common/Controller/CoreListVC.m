...
-(void)workBegin{
    ...
    if(self.canSetupHeaderRefreshControl){
        //添加顶部刷新控件
        [self.scrollView addHeaderWithTarget:self action:@selector(headerRefreshAction) customStr:_configModel.customeEndStatusMsg];
    }
...
//    }
    //出现error
    if (obj[@"rtnCode"]&&!([obj[@"rtnCode"] integerValue] ==1)) {
        dispatch_async(dispatch_get_main_queue(), ^{
