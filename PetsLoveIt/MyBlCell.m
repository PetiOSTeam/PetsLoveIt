...
-(void)dataFill{
    ...
    [self.sourceTitleview setTextContainerInset:UIEdgeInsetsMake(-2, 3, -2, 0)];//设置UITextView的内边距
    
    [self.sourceTitleview setTextAlignment:NSTextAlignmentLeft];//并设置左对齐
    if (Reasonsize.height < self.sourceTitleview.height) {
        CGFloat height = self.sourceTitleview.height - Reasonsize.height +8-3.5;
...
+(CGFloat)heightForCellWithObject:(BLModel *)object{
    ...
    if (textSize.height < 86) {
        return 261 - 86 + textSize.height - 8 + 3.5;
