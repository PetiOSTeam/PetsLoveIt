...
-(void)adjustLineViewFrameWithBtn:(CorePagesBarBtn *)btn isFirstBtn:(BOOL)isFirstBtn{
    ...
    //取出btn最小的x和宽度
    CGFloat minX=CGRectGetMinX(btn.frame)+(btn.width-btn.titleLabel.width)/2;
    if(minX<0) minX=CorePagesBarScrollMargin+(btn.width-btn.titleLabel.width)/2;
    
    CGFloat width=btn.titleLabel.frame.size.width;
...
-(UIView *)lineView{
    ...
    if(!_lineView){
        ...
        //设置颜色
        _lineView.backgroundColor=mRGBToColor(0xff4401);
